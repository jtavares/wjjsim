...
public class Sequential2ndPriceAuctionMDPClient {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while ((fromServer = in.readLine()) != null) {
        	...
        	//***********************************************************************
                ...
                int[] max_iterations = {5, 1, 1, 1, 1, 1, 1, 1};
        		// Let's feed everyone the same input price prediction		
        		ArrayList<Double> weight = new ArrayList<Double>();	// dummy weights
        		ArrayList<ArrayList<DiscreteDistribution>> tmp = new ArrayList<ArrayList<DiscreteDistribution>>(1);
        		// ???I have no checked the PricePredictor in the genPrediction method yet.
        		tmp.add(genPrediction(max_iterations[0], false, weight));
    		
        		agent = new SeqSSMDPAgent(agent_id, valuationMethod, tmp.get((int)(Math.random() )));
        		agents.add( agent);
		    	...
		    	for(int auction_id=0;auction_id<valuations.length;auction_id++)
		    	{
	        		...
	        		agent.postResult(new Result(auctions.get(auction_id), false, 0, ask_price, 0, ask_epsilon));
		    	}
        	...
        	//***********************************************************************
        		...
        		*/
        		...
        		///////////////////////////////////////////////
        		out.println(""+myBids); //Send agent's bids to server (as a string)
        	...
        	//Observe the state of auction variables. Store information locally
        		...
        		if (currentRound>0)
        		{
		        		...
		        		int  ask_epsilon=1;
		        		double ask_price=prices[currentRound-1]+ask_epsilon;
		        		
		        		agent.postResult(new Result(auctions.get(currentRound-1), is_winner, 0, ask_price, 0, ask_epsilon));
    ...
    private static ArrayList<DiscreteDistribution> genPrediction(int max_iterations, boolean smoothed, ArrayList<Double> weight) {
